...
function RouteComponent() {
  ...
  const comment = useMemo(
    () => posts.find((comment) => comment._id === id),
    [id],
  )
  ...
  const memoAddNewComment = useMemo(() => {
    ...
    return (
      <div className="w-full flex flex-col items-start pt-[12px] pl-[16px] pb-[12px] pr-[16px] mt-[10px] gap-[12px] border-1 border-[var(--color-background-16)] rounded-[22px] bg-[var(--color-background-14)]">
        ...
        <textarea
          className="w-full input-search-community-comment"
          placeholder={'Написати коментар...'}
          value={valueSearch}
          onChange={onHandlerSearchValue}
        />
  ...
  if (!comment)
    return (
      <h1 className="text-[32px] font-bold text-[var(--color-background)]">
        Пост не знайдено
      </h1>
    )
