...
import { FaWindows, FaPlaystation, FaXbox, FaApple, FaChevronDown } from 'react-icons/fa'
...
function RouteComponent() {
    ...
    if (platformInfo?.data && !selectedPlatform) {
      ...
      if (platforms.length > 0) {
        ...
        const defaultPlatform = platforms.find(p => 
          p.toLowerCase().includes('windows') || p.toLowerCase().includes('pc')
        ) || platforms[0]
  ...
  const { pcCharacteristics, consoleFeatures, availablePlatforms } = platformInfo.data
  ...
  const getPlatformInfo = (platform: string) => {
    ...
    if (lower.includes('windows') || (lower.includes('pc') && !lower.includes('playstation'))) {
      return {
  ...
  const isPcPlatform = (platform: string) => {
    const lower = platform.toLowerCase()
    return lower.includes('windows') || lower.includes('pc') || 
           lower.includes('mac') || lower.includes('apple')
  }
  ...
  const currentConsoleData = consoleFeatures.find(cf => 
    cf.platform.toLowerCase() === selectedPlatform.toLowerCase()
  )
    ...
    <>
      ...
      <div className="relative w-[50%]" ref={dropdownRef}>
        <button
          ...
          className="w-full flex items-center justify-between h-[48px] py-[12px] px-[20px] rounded-[20px] bg-[var(--color-background-15)] text-[var(--color-background)] border-1 border-[var(--color-background-16)] mt-[8px] cursor-pointer hover:bg-[var(--color-background-16)] transition-colors"
        >
          <div className="flex items-center gap-[10px]">
            ...
            <p className="text-[20px] font-medium font-manrope">
              {selectedPlatform ? getPlatformInfo(selectedPlatform).displayName : 'Windows'}
            </p>
          </div>
          <FaChevronDown className={`text-[var(--color-background)] transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
        </button>
        ...
        <>
          <div className="w-full flex gap-[64px] mt-[16px] text-[var(--color-background)]">
            ...
            <div className="w-[50%] flex flex-col gap-[16px] text-[20px]">
              ...
              <div className="flex flex-col">
                <p className="font-bold">{t('characteristics.soundCard')}</p>
                <p className="font-light">{currentPcData.recommendedAudioCard}</p>
              </div>
            </div>
          </div>
          ...
          {(currentPcData.langAudio.length > 0 || currentPcData.langText.length > 0) && (
            <div className="w-full mt-[32px] text-[var(--color-background)]">
              <div className="flex flex-col gap-[16px] text-[20px]">
                <p className="text-[24px] font-medium font-manrope">
                ...
                {currentPcData.langAudio.length > 0 && (
                  <div className="flex flex-col">
                    ...
                    <p className="font-light">{currentPcData.langAudio.join(', ')}</p>
                  </div>
                )}
                {currentPcData.langText.length > 0 && (
                  <div className="flex flex-col">
                    <p className="font-bold">{t('characteristics.text')}</p>
                    <p className="font-light">{currentPcData.langText.join(', ')}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {!isPcPlatform(selectedPlatform) && currentConsoleData && (
        <div className="w-full mt-[16px] text-[var(--color-background)]">
          <div className="flex flex-col gap-[24px] text-[20px]">
            <p className="text-[24px] font-medium font-manrope">
            ...
            {currentConsoleData.resolution && (
              <div className="flex flex-col">
                <p className="font-bold">{t('characteristics.resolution', 'Resolution')}</p>
            ...
            {currentConsoleData.frameRate && (
              <div className="flex flex-col">
                <p className="font-bold">{t('characteristics.frameRate', 'Frame Rate')}</p>
            ...
            {currentConsoleData.performanceModes && (
              <div className="flex flex-col">
                <p className="font-bold">{t('characteristics.performanceModes', 'Performance Modes')}</p>
                <p className="font-light">{currentConsoleData.performanceModes}</p>
              </div>
            )}

            <div className="flex flex-col">
              <p className="font-bold">{t('characteristics.hdrSupport', 'HDR Support')}</p>
              <p className="font-light">{currentConsoleData.hdrSupport ? t('common.yes', 'Yes') : t('common.no', 'No')}</p>
            </div>

            <div className="flex flex-col">
              <p className="font-bold">{t('characteristics.rayTracing', 'Ray Tracing')}</p>
              <p className="font-light">{currentConsoleData.rayTracingSupport ? t('common.yes', 'Yes') : t('common.no', 'No')}</p>
            </div>

            {currentConsoleData.controllerFeatures && (
              <div className="flex flex-col">
                <p className="font-bold">{t('characteristics.controllerFeatures', 'Controller Features')}</p>
                <p className="font-light">{currentConsoleData.controllerFeatures}</p>
              </div>
            )}
            ...
            {currentConsoleData.storageRequired && (
              <div className="flex flex-col">
                <p className="font-bold">{t('characteristics.storage')}</p>
                <p className="font-light">{currentConsoleData.storageRequired}</p>
              </div>
            )}

            <div className="flex flex-col">
              <p className="font-bold">{t('characteristics.onlineRequired', 'Online Play Required')}</p>
              <p className="font-light">{currentConsoleData.onlinePlayRequired ? t('common.yes', 'Yes') : t('common.no', 'No')}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!currentPcData && !currentConsoleData && (
        <div className="w-full mt-[16px] text-[var(--color-background)]">
          <p className="text-[20px]">
            {t('characteristics.noDataForPlatform', 'No characteristics available for this platform.')}
          </p>
        </div>
      )}
    </>
