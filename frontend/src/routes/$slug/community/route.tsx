...
function RouteComponent() {
  ...
  const onHedlerSearchValue = (e: ChangeEvent<HTMLInputElement>) => setValueSearch(e.target.value)
    ...
    <div className="w-full flex flex-row gap-[24px]">
      <div className="w-[75%] flex flex-col gap-[8px] min-w-0 mb-[256px] justify-start">
        ...
        <div className="w-full flex flex-col gap-[24px]">
          {posts.filter(elem => elem.type === postType).map((el, i) => <GamePost key={el._id} {...el} isNext={true} />)}
        </div>
      </div>
      <div className="w-[25%] flex flex-col gap-[42px] min-w-[338px] mb-[256px]">
        ...
        <div className="w-full flex flex-col p-[20px] bg-[var(--color-background-16)] rounded-[20px]">
          <div className='flex items-center gap-[6px] relative'>
            ...
            <div className='search-select-g'>
              <div className='flex items-center gap-[6px]'>
                <p className='text-[var(--color-background)] text-[14px] cursor-pointer'>{postTypeSort.showText}</p>
          ...
          <input className='w-full input-search-community' placeholder={'Пошук: ' + postType} value={valueSearch} onChange={onHedlerSearchValue} />
          <div className='w-full flex flex-col gap-[8px]'>
            {rosdils.map((text) => <p key={text} onClick={() => onHendlerPost(text)} className={'rounded-[12px] p-[9px] pl-[12px] pr-[12px] text-[var(--color-background)] text-search-community ' + (text === postType ? 'bg-[var(--color-background-18)]' : '')}>{text}</p>)}
          </div>
        </div>
      </div>
      {/* <p className="text-[32px] font-bold text-[var(--color-background)]">
