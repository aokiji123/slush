...
function RouteComponent() {
  ...
  const [commentSortBy, setCommentSortBy] = useState<'newest' | 'likes'>(
    'newest',
  )
  ...
  const handleLikeComment = async (commentId: string) => {
    ...
    const previousLikesCount =
      ...
      comments?.find((c) => c.id === commentId)?.likesCount ||
      0
  ...
  useEffect(() => {
    const savedSort = localStorage.getItem('commentSortBy') as
      | 'newest'
      | 'likes'
      | null
    if (savedSort) {
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-[24px] px-4 lg:px-0">
      {/* Left Column - Main Content */}
      <div className="w-full lg:w-[1092px] flex flex-col gap-[16px] md:gap-[24px]">
        {/* Header Section */}
          ...
          <div className="flex items-center gap-[12px] md:gap-[16px]">
            ...
            <div className="flex items-center gap-[12px] md:gap-[16px]">
              <button
                ...
                className="bg-[#004252] rounded-[20px] md:rounded-[24px] flex items-center gap-[8px] md:gap-[12px] pr-[12px] md:pr-[16px] hover:bg-[#005a6b] transition-colors cursor-pointer"
              >
                <img
                  src={post.authorAvatar ?? '/avatar.png'}
                  alt={post.authorUsername ?? 'User'}
                  className="w-[36px] h-[36px] md:w-[48px] md:h-[48px] rounded-full"
                />
                <span className="text-[16px] md:text-[20px] font-bold text-[#f1fdff] font-artifakt">
                  {post.authorUsername ?? 'Unknown User'}
                </span>
              </button>
              <span className="text-[12px] md:text-[14px] text-[rgba(204,248,255,0.65)] font-artifakt hidden sm:inline">
        ...
        <div className="flex flex-col gap-[12px] md:gap-[16px]">
          ...
          <div className="flex flex-col gap-[12px]">
            <div className="flex flex-col sm:flex-row gap-[8px] sm:gap-[10px] items-start sm:items-center">
              <span className="text-[14px] md:text-[16px] text-[rgba(204,248,255,0.65)] font-artifakt">
                Сортування:
              </span>
              <div className="relative sort-dropdown">
                <button
                  onClick={() => setIsSortDropdownOpen(!isSortDropdownOpen)}
                  className="flex items-center gap-[2px] text-[#f1fdff] text-[14px] md:text-[16px] font-artifakt font-medium hover:text-[#37c3ff] transition-colors"
                >
                  <span>
                  ...
                  <svg
                    className={`w-5 h-5 md:w-6 md:h-6 transition-transform ${isSortDropdownOpen ? 'rotate-180' : ''}`}
                    viewBox="0 0 24 24"
                    fill="none"
                  >
                    <path
                ...
                {/* Dropdown Menu */}
                {isSortDropdownOpen && (
                  <div className="absolute top-full mt-[4px] left-0 bg-[#004252] border border-[#046075] rounded-[8px] p-[12px] flex flex-col gap-[2px] min-w-[160px] md:min-w-[200px] z-10">
                    <button
                      onClick={() => {
                        setCommentSortBy('newest')
                        setIsSortDropdownOpen(false)
                      }}
                      className={`px-[12px] py-[6px] rounded-[20px] text-[14px] md:text-[16px] text-[#f1fdff] font-artifakt text-left transition-colors ${
                        commentSortBy === 'newest'
                          ? 'bg-[rgba(55,195,255,0.25)]'
                          : 'hover:bg-[rgba(55,195,255,0.15)]'
                      }`}
                    >
                      Спочатку нові
                    </button>
            ...
            {/* Comment Input */}
            <div className="bg-[rgba(0,20,31,0.4)] border border-[#046075] rounded-[16px] md:rounded-[22px] px-[12px] md:px-[16px] py-[12px] flex flex-col gap-[8px]">
              <textarea
                value={commentText}
                onChange={(e) => setCommentText(e.target.value)}
                placeholder="Написати коментар..."
                className="flex-1 bg-transparent text-[#f1fdff] placeholder-[rgba(204,248,255,0.65)] text-[14px] md:text-[16px] font-artifakt outline-none resize-none min-h-[44px] max-h-[180px]"
                rows={3}
              />
              {commentText.trim() && (
                <div className="flex flex-col sm:flex-row gap-[12px] items-stretch sm:items-center justify-end">
                  <button
                    onClick={() => setCommentText('')}
                    className="px-[8px] py-[8px] text-[#ffa599] text-[14px] md:text-[16px] font-artifakt font-medium order-2 sm:order-1"
                  >
                    Відхилити
                  </button>
                  <button
                    ...
                    className="bg-[#24e5c2] text-[#00141f] rounded-[16px] md:rounded-[20px] px-[16px] md:px-[20px] py-[8px] h-[40px] text-[14px] md:text-[16px] font-artifakt font-medium disabled:opacity-50 order-1 sm:order-2"
                  >
                ...
                .map((comment) => {
                  ...
                  return (
                    <div key={comment.id} className="flex flex-col gap-[12px]">
                      {/* Main Comment */}
                      <div className="bg-[#002f3d] rounded-[16px] md:rounded-[20px] p-[16px] md:p-[24px] flex flex-col gap-[16px] md:gap-[20px]">
                        <div className="flex flex-col gap-[12px] md:gap-[16px]">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-[8px] md:gap-[12px]">
                              <button
                                onClick={() =>
                                  navigate({
                                    to: '/profile/$nickname',
                                ...
                                className="bg-[#004252] rounded-[16px] md:rounded-[20px] flex items-center gap-[8px] md:gap-[12px] pr-[12px] md:pr-[16px] hover:bg-[#005a6b] transition-colors cursor-pointer"
                              >
                                <img
                                  src={comment.authorAvatar || '/avatar.png'}
                                  alt={comment.authorUsername || 'User'}
                                  className="w-[32px] h-[32px] md:w-[36px] md:h-[36px] rounded-full"
                                />
                                <span className="text-[14px] md:text-[16px] font-bold text-[#f1fdff] font-artifakt">
                                  {comment.authorUsername || 'Unknown User'}
                                </span>
                              </button>
                              <span className="text-[12px] md:text-[14px] text-[rgba(204,248,255,0.65)] font-artifakt">
                          ...
                          <div className="flex flex-col gap-[8px]">
                            <p className="text-[14px] md:text-[16px] text-[#f1fdff] font-artifakt leading-[1.5] tracking-[-0.16px]">
                              {comment.content}
                            </p>
                          </div>
                        </div>

                        <div className="flex flex-wrap gap-[8px] md:gap-[12px] items-center">
                          <button
                            ...
                            >
                              ...
                              <p>
                                {collapsedComments.has(comment.id)
                                  ? `Показати ${replies.length} відповідей`
                                  : 'Приховати відповіді'}
                              </p>
                            </button>
                          )}
                      ...
                      {replies.length > 0 &&
                        !collapsedComments.has(comment.id) && (
                          <div className="ml-[12px] md:ml-[24px] flex flex-col gap-[12px]">
                            {replies.map((reply) => {
                              ...
                              return (
                                <div
                                  key={reply.id}
                                  className="bg-[#002f3d] rounded-[16px] md:rounded-[20px] p-[16px] md:p-[24px] flex flex-col gap-[16px] md:gap-[20px] border-l-[6px] md:border-l-[8px] border-[#24e5c2]"
                                >
                                  <div className="flex flex-col gap-[12px] md:gap-[16px]">
                                    <div className="flex items-start justify-between gap-4">
                                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-[8px] md:gap-[12px]">
                                        <button
                                          ...
                                          className="bg-[rgba(55,195,255,0.25)] rounded-[16px] md:rounded-[20px] flex items-center gap-[8px] md:gap-[12px] pr-[12px] md:pr-[16px] hover:bg-[rgba(55,195,255,0.35)] transition-colors cursor-pointer"
                                        >
                                          <img
                                            src={
                                              reply.authorAvatar ||
                                    ...
                                    <div className="flex flex-col gap-[6px] md:gap-[8px]">
                                      ...
                                      {parentComment && (
                                        <div className="bg-[rgba(55,195,255,0.1)] rounded-[10px] md:rounded-[12px] p-[10px] md:p-[12px] mb-[6px] md:mb-[8px] border-l-[3px] md:border-l-[4px] border-[#37c3ff]">
                                          <div className="text-[10px] md:text-[12px] text-[#37c3ff] mb-[4px] font-artifakt font-medium">
                                            Replying to @
                                            {parentComment.authorUsername}
                                          </div>
                                          <div className="text-[12px] md:text-[14px] text-[rgba(204,248,255,0.8)] font-artifakt italic">
                                            "
                                            {truncateText(
      ...
      <div className="w-full lg:w-[348px] flex flex-col gap-[24px] md:gap-[32px]">
        {/* Game Info Section */}
        <div className="flex flex-col gap-[16px] md:gap-[20px]">
          <div className="flex flex-col gap-[8px]">
            <h2 className="text-[14px] md:text-[16px] font-bold text-[#f1fdff] font-artifakt">
              {post?.title || 'Завантаження...'}
            </h2>
            <div className="flex gap-[16px] md:gap-[32px] flex-wrap">
              <div className="flex gap-[4px] text-[12px] md:text-[14px] font-artifakt">
                <span className="font-bold text-[#f1fdff]">
                  {comments?.length ? Math.floor(comments.length * 12.5) : '0'}
                </span>
                <span className="text-[rgba(204,248,255,0.65)]">
                  підписників
                </span>
              </div>
              <div className="flex gap-[4px] items-center">
                <span className="text-[12px] md:text-[14px] font-bold text-[#f1fdff] font-artifakt">
                  {comments?.length || 0}
                </span>
                <div className="w-[6px] h-[6px] md:w-[8px] md:h-[8px] bg-[#24e5c2] rounded-full"></div>
              </div>
            </div>
          </div>

          <div className="flex gap-[8px] items-center">
            <button className="flex-1 bg-[#24e5c2] text-[#00141f] rounded-[16px] md:rounded-[20px] px-[16px] md:px-[20px] py-[8px] flex items-center gap-[8px] justify-center h-[40px]">
              <svg
                className="w-5 h-5 md:w-6 md:h-6"
                viewBox="0 0 24 24"
                fill="none"
              ...
              <span className="text-[14px] md:text-[16px] font-artifakt font-medium">
                Створити пост
              </span>
            </button>

            <button className="bg-[#046075] rounded-[16px] md:rounded-[20px] p-[8px] w-[40px] h-[40px] flex items-center justify-center">
        ...
        <div className="flex flex-col gap-[12px] md:gap-[16px]">
          <h3 className="text-[18px] md:text-[20px] font-bold text-[#f1fdff] font-manrope">
            Інші обговорення
          </h3>
