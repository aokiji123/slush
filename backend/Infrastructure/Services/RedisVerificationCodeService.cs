...
public interface IRedisVerificationCodeService
{
    Task<string> GenerateAndStoreCodeAsync(string email, string purpose, TimeSpan? expiration = null);
    Task<bool> VerifyCodeAsync(string email, string code, string purpose);
    Task RemoveCodeAsync(string email, string purpose);
    Task<bool> IsCodeValidAsync(string email, string purpose);
}

public class RedisVerificationCodeService : IRedisVerificationCodeService
{
    private readonly IRedisCacheService _cacheService;
    private readonly TimeSpan _defaultExpiration = TimeSpan.FromMinutes(10);

    public RedisVerificationCodeService(IRedisCacheService cacheService)
    {
