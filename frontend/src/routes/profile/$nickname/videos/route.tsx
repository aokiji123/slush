...
export const Route = createFileRoute('/profile/$nickname/videos')({
...
function ProfileVideosPage() {
  ...
  // Fetch user's posts (videos only)
  const { data: userPosts, isLoading: isLoadingPosts, isError: isPostsError } = useUserPosts(profileUser?.id || '', 'Video', sortBy)
  ...
  const videoPosts = userPosts?.filter(post => post.type === PostType.Video) || []

  // Client-side search filtering
  const filteredItems = videoPosts.filter((post) => {
            ...
            <div style={{ width: '1092px' }}>
              ...
              <div className="bg-[#004252] p-[20px] rounded-[20px] flex flex-col gap-[12px]">
                ...
                <ProfileTabToolbar
                  ...
                  searchPlaceholder={t('profile.searchPlaceholders.videos')}
                  sortBy={sortBy}
